...
public class JuntaResultadosFutureWSFutureBanco implements Callable<Void> {
	...
	public Void call() {
		...
		try {
			String numeroMagico = this.futureWS.get(20, TimeUnit.SECONDS);// estamos aguardando os resultados de cada
																			// Future por 20 segundos. senão, lança
																			// timeout
			String numeroMagico2 = this.futureBanco.get(20, TimeUnit.SECONDS);

			this.saidaCliente.println("Resultado do comando c2: " + numeroMagico + ", " + numeroMagico2);// . Após isso,
																											// estamos
																											// juntando
																											// os
																											// resultados
